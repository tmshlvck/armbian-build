...
function get_docker_info_once() {
	if [[ -z "${DOCKER_INFO}" ]]; then
		...
		#
		# Key Podman-specific configurations:
		# - Runs with sudo (root mode) for privileged operations
		# - Uses --network host for proper networking
		# - Requires mount options: suid,dev for correct file permissions
